function generateDivisionQuestions(numQuestions) {
    ...
    for (let i = 0; i < numQuestions; i++) {
        // Gera dois números aleatórios para a multilicação
        const num1 = Math.floor(Math.random() * 200) + 2;
        const num2 = Math.floor(Math.random() * 10) + 2; // 
        const correctAnswer = Math.floor(num1 * num2); // Usa multiplicação inteira

        // Cria a pergunta
        const questionText = `Qual é o resultado de ${num1} x ${num2}?`;

        // Gera respostas aleatórias
        const answers = [correctAnswer];
        while (answers.length < 4) {
            const randomAnswer = Math.floor(Math.random() * 2000) + 2;
...
// Cria 10 perguntas de multiplicação
